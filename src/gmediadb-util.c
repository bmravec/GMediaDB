...
void info_clicked (GtkWidget *widget, gpointer user_data);
...
static GtkWidget *import_button, *remove_button, *quit_button, *info_button;
...
main (int argc, char *argv[])
{
    ...
    info_button = gtk_button_new_with_label ("Get Info");
...
//        gtk_tree_selection_set_mode (sel, GTK_SELECTION_MULTIPLE);
    ...
    gtk_box_pack_start (GTK_BOX (hbox), info_button, FALSE, FALSE, 0);
    ...
    g_signal_connect (G_OBJECT (info_button), "clicked", G_CALLBACK (info_clicked), NULL);
...
void
info_clicked (GtkWidget *widget, gpointer user_data)
{
    g_print ("InfoClicked\n");
    MediaObject *mo;
    gint id;
    ...
    gint i, pn = gtk_notebook_get_current_page (GTK_NOTEBOOK (notebook));
    for (i = 0; i < MOS_NUMBER; i++) {
        if (mos[i].page_number == pn) {
            mo = &mos[i];
        }
    ...
    gtk_tree_model_get (GTK_TREE_MODEL (mo->store), &iter, 0, &id, -1);
    
    g_print ("ID: %d\n", id);
    
    if (!dbus_g_proxy_call (mo->proxy, "get_entry_tags", &error,
        G_TYPE_UINT, id, G_TYPE_INVALID,
        G_TYPE_STRV, &tags, G_TYPE_INVALID)) {
        if (error->domain == DBUS_GERROR && error->code == DBUS_GERROR_REMOTE_EXCEPTION) {
            g_printerr ("Caught remote method exception %s: %s",
                dbus_g_error_get_name (error), error->message);
